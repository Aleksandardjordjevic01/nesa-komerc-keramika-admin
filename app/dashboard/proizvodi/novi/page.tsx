...
export default function ProductCreatePage() {
  ...
  async function handleSave() {
    if (!form.name.trim()) { alert('Naziv proizvoda je obavezan.'); return; }
    ...
    try {
      ...
      setSaveOk(true);
      setTimeout(() => router.push(`/dashboard/proizvodi/${created.id}`), 800);
    } catch (e) {
      alert(e instanceof Error ? e.message : 'Greška pri kreiranju proizvoda');
    } finally {
      setSaving(false);
    }
  }
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-lg font-semibold text-foreground truncate">{form.name || 'Novi proizvod'}</h1>
              <p className="text-xs text-muted-foreground">Kreiranje novog proizvoda</p>
            </div>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            ...
            >
              ...
              {saving ? 'Kreiranje...' : saveOk ? 'Kreirano!' : 'Kreiraj proizvod'}
            </button>
          </div>
        </div>
